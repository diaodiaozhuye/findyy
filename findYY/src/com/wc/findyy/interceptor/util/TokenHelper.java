package com.wc.findyy.interceptor.util;
...
public class TokenHelper {
     ...
     * 保存token值的默认命名空间 
     */  
    public static final String TOKEN_NAMESPACE = "hnjh_tokens";  
     ...
     * 持有token名称的字段名 
     */  
    public static final String TOKEN_NAME_FIELD = "hnjh_token_name";  //如果改为hnjh.token.name 页面用${requestScope.hnjh.token.name}获取不到值
   ...
   /* private static IRedisCacheClient redisCacheClient;// 缓存调用,代替session,支持分布式  
    ...
    {  
        ...
        {  
            ...
            session.setAttribute(tokenName0, token);
...
/*        for(Object key: params.keySet()){
    ...
    {  
       ...
       // String cacheToken = redisCacheClient.listLpop(tokenCacheName);
        String cacheToken = (String) session.getAttribute(tokenCacheName);
        ...
        session.removeAttribute(tokenCacheName);
        return true;  
    }  
